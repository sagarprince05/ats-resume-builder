...
Groq key on the server. Visitors open a link, upload a CV, paste a posting
...
ceiling. Both hosts sit far above it.
...
The relay lives in `functions/api/groq/[[path]].js` and is deployed
...
their own free Groq key (the app shows the "Add your free Groq API key"
...
runs through the relay. `https://your-site/api/groq/health` should show
`{"ok":true,"relay":"groq","keySet":true}`. If `keySet` is false, the
variable was not saved or the site was not redeployed after adding it.

## Notes

- **Quota.** Everyone who uses the site shares your Groq quota. The relay
...
- **Without the relay** (for example on GitHub Pages, which cannot run
  ...
  missing and asks each visitor for their own free Groq key instead.
- **Privacy.** Resumes are processed in the visitor's browser and sent only
  to Groq for the rewrite. Nothing is stored on the host.
